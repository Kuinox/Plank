...
public sealed class ParquetRowGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<MappedColumn> columns)
    {
        var namespaceName = schemaType.ContainingNamespace is { IsGlobalNamespace: false }
            ...
            : null;
        var schemaReaderTypeName = $"{schemaType.Name}Reader";
        var schemaFooterTypeName = $"{schemaType.Name}Footer";
        var schemaRowGroupReaderTypeName = $"{schemaType.Name}RowGroup";
        ...
        builder.AppendLine("        => new(stream, options ?? global::Plank.Writing.ParquetWriterOptions.Default);");
        builder.AppendLine();
        builder.Append("    public static ").Append(schemaReaderTypeName)
            .AppendLine(" CreateReader(global::System.IO.Stream stream, global::Plank.Reading.ParquetReaderOptions? options = null)");
        builder.AppendLine("        => new(Schema.CreateReader(stream, options ?? global::Plank.Reading.ParquetReaderOptions.Default));");
        builder.AppendLine();
        builder.Append("    public static ").Append(schemaReaderTypeName)
            .AppendLine(" CreateReader(global::Plank.Reading.ParquetReader reader)");
        builder.AppendLine("        => new(reader);");
        builder.AppendLine();
        builder.Append("    public sealed class ").Append(schemaReaderTypeName)
        ...
        builder.AppendLine();
        builder.Append("        internal ").Append(schemaReaderTypeName)
            .AppendLine("(global::Plank.Reading.ParquetReader reader)");
        builder.AppendLine("        {");
        builder.AppendLine("            reader = reader ?? throw new global::System.ArgumentNullException(nameof(reader));");
        builder.AppendLine("            if (!global::System.Object.ReferenceEquals(reader.Schema, Schema))");
        builder.AppendLine("                throw new global::System.ArgumentException(\"Reader schema does not match this generated schema.\", nameof(reader));");
        builder.AppendLine();
        builder.AppendLine("            _reader = reader;");
        builder.AppendLine("            _disposed = false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.Append("        public ").Append(schemaFooterTypeName).AppendLine(" Footer");
        builder.AppendLine("            => new(_reader.Footer);");
        builder.AppendLine();
        builder.AppendLine("        public void Reset(global::System.IO.Stream stream)");
        builder.AppendLine("            => _reader.Reset(stream);");
        builder.AppendLine();
        builder.Append("        public ").Append(schemaRowGroupReaderTypeName)
            .AppendLine(" OpenRowGroup(global::System.IO.Stream stream, global::Plank.Reading.RowGroupToken token)");
        builder.AppendLine("            => new(_reader.OpenRowGroup(stream, token));");
        builder.AppendLine();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.Append("    public readonly struct ").Append(schemaFooterTypeName).AppendLine();
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Reading.ParquetFooter _footer;");
        builder.AppendLine();
        builder.Append("        internal ").Append(schemaFooterTypeName)
            .AppendLine("(global::Plank.Reading.ParquetFooter footer)");
        builder.AppendLine("        {");
        builder.AppendLine("            _footer = footer;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public global::Plank.Reading.ParquetFileMetadata Metadata");
        builder.AppendLine("            => _footer.Metadata;");
        builder.AppendLine();
        builder.AppendLine("        public global::Plank.Reading.RowGroupTokenEnumerable EnumerateRowGroups()");
        builder.AppendLine("            => _footer.EnumerateRowGroups();");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.Append("    public sealed class ").Append(schemaRowGroupReaderTypeName)
            .AppendLine(" : global::System.IDisposable");
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Reading.RowGroupReader _rowGroupReader;");
        builder.AppendLine();
        builder.Append("        internal ").Append(schemaRowGroupReaderTypeName)
            .AppendLine("(global::Plank.Reading.RowGroupReader rowGroupReader)");
        builder.AppendLine("        {");
        builder.AppendLine("            _rowGroupReader = rowGroupReader ?? throw new global::System.ArgumentNullException(nameof(rowGroupReader));");
        builder.AppendLine("        }");
        builder.AppendLine();
        for (var i = 0; i < columns.Length; i++)
        {
            builder.Append("        public global::Plank.Reading.ColumnReader<")
                .Append(columns[i].ClrTypeName).Append("> ")
                .Append(columns[i].PropertyName).Append(" => _rowGroupReader.CreateColumnReader<")
        ...
        builder.AppendLine();
        builder.AppendLine("        public void Dispose()");
        builder.AppendLine("            => _rowGroupReader.Dispose();");
        builder.AppendLine("    }");
        builder.AppendLine();
