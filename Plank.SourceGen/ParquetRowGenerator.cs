...
{
    static readonly SymbolDisplayFormat EscapedFullyQualifiedTypeNameFormat =
        ...
        ImmutableArray<MappedColumn> columns)
    {
        ...
        var namespaceName = GetNamespaceName(schemaType.ContainingNamespace);
        var rowTypeName = namespaceName is null
            ? $"global::{EscapeIdentifier(schemaType.Name)}"
            : $"global::{namespaceName}.{EscapeIdentifier(schemaType.Name)}";
        ...
        builder.AppendLine("    ]);");
        builder.AppendLine();
        builder.Append("    public delegate global::System.ReadOnlySpan<byte> ").Append(names.Root("Route")).Append('(')
            .Append(rowTypeName)
            .AppendLine(" row, global::Plank.IParquetBufferPool bufferPool, out global::Plank.ParquetBuffer? allocation);");
        builder.AppendLine();
        builder.Append("    public static ").Append(names.Root("DatasetWriter")).Append(" " + names.Root("CreateDatasetWriter") + "<TFile>(")
            .Append(names.Root("Route"))
        ...
        builder.AppendLine("            options ?? global::Plank.Dataset.DatasetWriterOptions.Default);");
        builder.AppendLine();
        builder.Append("    public static ").Append(names.Root("Writer"))
            .AppendLine(" " + names.Root("CreateRowWriter") + "(global::Plank.Writing.RowGroupWriter rowGroupWriter, global::Plank.Writing.ParquetWriterOptions? options = null)");
        ...
        builder.AppendLine();
        builder.Append("    public static ").Append(names.Root("Reader"))
            .AppendLine(" " + names.Root("CreateReader") + "(global::System.IO.Stream stream, global::Plank.Reading.Logical.ParquetReaderOptions? options = null, global::Plank.Reading.Logical.ParquetPagePruner? pagePruner = null)");
        builder.Append("        => new(").Append(names.Root("Schema")).AppendLine(".CreateReader(stream, options, pagePruner));");
        builder.AppendLine();
        builder.Append("    public static ").Append(names.Root("Reader"))
            .AppendLine(" " + names.Root("CreateReader") + "(global::Plank.Reading.IParquetReadSource source, global::Plank.Reading.Logical.ParquetReaderOptions? options = null, global::Plank.Reading.Logical.ParquetPagePruner? pagePruner = null)");
        builder.Append("        => new(").Append(names.Root("Schema")).AppendLine(".CreateReader(source, options, pagePruner));");
        builder.AppendLine();
        AppendColumnReader(builder, names, columns, names.Root("Schema"), names.Root("Reader"));
        builder.AppendLine();
        AppendRowApiProjection(builder, names, columns);
        builder.AppendLine();
        AppendRowApiColumnDescriptors(builder, names, columns, names.Root("Schema"));
        builder.AppendLine();
        builder.AppendLine("    public static " + names.Root("SchemaWriter") + " " + names.Root("CreateWriter") + "(global::System.IO.Stream stream, global::Plank.Writing.ParquetWriterOptions? options = null)");
        builder.AppendLine("        => new(stream, options ?? global::Plank.Writing.ParquetWriterOptions.Default);");
        builder.AppendLine();
        builder.AppendLine("    public sealed class " + names.Root("SchemaWriter") + " : global::System.IDisposable");
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Writing.ParquetWriter " + names.Helper("_writer") + ";");
        builder.AppendLine();
        builder.AppendLine("        internal " + names.Root("SchemaWriter") + "(global::System.IO.Stream stream, global::Plank.Writing.ParquetWriterOptions options)");
        builder.AppendLine("        {");
        ...
        builder.Append("            " + names.Helper("_writer") + " = ").Append(names.Root("Schema")).AppendLine(".CreateWriter(stream, options);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public " + names.Root("RowGroup") + " StartRowGroup()");
        builder.AppendLine("            => new(" + names.Helper("_writer") + ".StartRowGroup());");
        builder.AppendLine();
        builder.AppendLine("        public void Reset(global::System.IO.Stream stream)");
        builder.AppendLine("            => " + names.Helper("_writer") + ".Reset(stream);");
        builder.AppendLine();
        builder.AppendLine("        public void CloseFile()");
        builder.AppendLine("            => " + names.Helper("_writer") + ".CloseFile();");
        builder.AppendLine();
        builder.AppendLine("        public void Dispose()");
        builder.AppendLine("            => " + names.Helper("_writer") + ".Dispose();");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    public sealed class " + names.Root("RowGroup"));
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Writing.RowGroupWriter " + names.Helper("_rowGroupWriter") + ";");
        for (var i = 0; i < columns.Length; i++)
            builder.Append("        readonly global::Plank.Writing.SerializedColumn<")
                .Append(columns[i].ClrTypeName).Append("> ")
                .Append(names.Get("buffer:" + columns[i].PropertyName, "_" + columns[i].PropertyName)).AppendLine(";");
        builder.AppendLine();
        builder.AppendLine("        internal " + names.Root("RowGroup") + "(global::Plank.Writing.RowGroupWriter rowGroupWriter)");
        builder.AppendLine("        {");
        builder.AppendLine("            " + names.Helper("_rowGroupWriter") + " = rowGroupWriter ?? throw new global::System.ArgumentNullException(nameof(rowGroupWriter));");
        for (var i = 0; i < columns.Length; i++)
            builder.Append("            ").Append(names.Get("buffer:" + columns[i].PropertyName, "_" + columns[i].PropertyName)).Append(" = rowGroupWriter.CreateSerializedColumn<")
                .Append(columns[i].ClrTypeName).Append(">(").Append(names.Root("Schema")).Append(".LeafColumns[")
        ...
        for (var i = 0; i < columns.Length; i++)
        {
            builder.Append("        public global::Plank.Writing.SerializedColumn<")
                .Append(columns[i].ClrTypeName).Append("> ")
                .Append(EscapeIdentifier(columns[i].PropertyName)).Append(" => ").Append(names.Get("buffer:" + columns[i].PropertyName, "_" + columns[i].PropertyName)).AppendLine(";");
            if (i < columns.Length - 1)
                builder.AppendLine();
        }
        builder.AppendLine();
        builder.AppendLine("        public void " + names.Get("row-group:Write", "Write") + "<T>(global::Plank.Writing.SerializedColumn<T> serialized)");
        builder.AppendLine("            => " + names.Helper("_rowGroupWriter") + ".Write(serialized);");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.Append("    public struct ").AppendLine(names.Root("Writer"));
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.RowApi.RowGroupWriterCore<" + names.Root("BufferSlot") + "> " + names.Helper("_core") + ";");

        builder.AppendLine();
        builder.Append("        internal ").Append(names.Root("Writer"))
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public " + names.Root("Row") + " GetRow()");
        builder.AppendLine("            => " + names.Helper("_core") + ".GetSlotForRow().GetRow();");
        builder.AppendLine();
        builder.AppendLine("        public void Write()");
        builder.AppendLine("            => " + names.Helper("_core") + ".Write();");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.Append("            : base(file, filePath, ").Append(names.Root("Schema"))
        ...
        builder.Append("            : base(stream, ").Append(names.Root("Schema"))
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        protected override " + names.Root("BufferSlot") + " CreateSlot(global::Plank.Writing.ParquetWriter writer)");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.Append("    public sealed class ").Append(names.Root("DatasetWriter"))
            .Append(" : global::Plank.Dataset.DatasetWriterBase<").Append(rowTypeName)
            .AppendLine(">, global::System.IDisposable");
        builder.AppendLine("    {");
        builder.Append("        readonly ").Append(names.Root("Route")).AppendLine(" " + names.Helper("_route") + ";");
        builder.AppendLine();
        builder.Append("        internal ").Append(names.Root("DatasetWriter")).Append('(').Append(names.Root("Route"))
            .AppendLine(" route, global::Plank.Writing.IParquetWriteSource[] files, global::Plank.Dataset.DatasetWriterOptions options)");
        builder.Append("            : base(").Append(names.Root("Schema"))
            .AppendLine(", " + names.Root("s_rowApiColumns") + ", options.WriterOptions.RowApiInitialRowCapacity, files, options)");
        builder.AppendLine("        {");
        ...
        builder.Append("            : base(").Append(names.Root("Schema"))
            .AppendLine(", " + names.Root("s_rowApiColumns") + ", options.WriterOptions.RowApiInitialRowCapacity, files, filePath, options)");
        builder.AppendLine("        {");
        builder.AppendLine("            " + names.Helper("_route") + " = route;");
        builder.AppendLine("            InitializeSlots();");
        builder.AppendLine("        }");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        AppendRowReader(builder, names, columns, names.Root("Schema"));
        builder.AppendLine();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        AppendWriteRow(builder, names, columns);
        RowCursorEmitter.AppendCursor(builder, names,
            columns.Select(static column => column.ClrTypeName).ToArray(),
            () => AppendWriteProperties(builder, names, columns,
                i => $"global::System.Runtime.CompilerServices.Unsafe.Add(ref {names.Helper("_buffers")}._column{i}, {names.Helper("_index")})", names.Helper("_buffers") + "." + names.Helper("_ownerSlot")));
        builder.AppendLine("}");

        return builder.ToString();
    }

    static void AppendWriteRow(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns)
    {
        ...
        builder.AppendLine();
        AppendWriteProperties(builder, names, columns,
            i => UncheckedBufferElement($"{names.Helper("_ownerSlot")}._column{i}", names.Helper("_index")), names.Helper("_ownerSlot"));
        builder.AppendLine("    }");
    }

    static void AppendWriteProperties(
        StringBuilder builder,
        GeneratedMemberNames names,
        ImmutableArray<MappedColumn> columns,
        ...
        string ownerSlot)
    {
        for (var i = 0; i < columns.Length; i++)
        {
            ...
            if (SupportsOwnerSetter(columns[i].ClrTypeName))
            {
                ...
                {
                    builder.Append("        public void ").Append(names.Get("setter:" + columns[i].PropertyName, "Set" + columns[i].PropertyName))
                    ...
                    builder.AppendLine("            {");
                    builder.Append("                ").Append(ownerSlot).AppendLine(".RegisterOwner(owner);");
                    builder.AppendLine("            }");
                    builder.AppendLine("        }");
                }
                else
    ...
    static void AppendRowApiColumnDescriptors(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns,
        string schemaMemberName)
    {
        for (var i = 0; i < columns.Length; i++)
        {
            builder.Append("    static readonly global::Plank.RowApi.RowApiColumnDescriptor<")
                .Append(columns[i].ClrTypeName).Append("> ")
                .Append(names.Helper(GetRowApiColumnFieldName(columns[i].PropertyName)))
        ...
        for (var i = 0; i < columns.Length; i++)
            builder.Append("        ").Append(names.Helper(GetRowApiColumnFieldName(columns[i].PropertyName))).AppendLine(",");
        builder.AppendLine("    ];");
    }

    static void AppendColumnReader(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns,
        string schemaMemberName, string readerTypeName)
    {
        builder.Append("    public sealed class ").Append(readerTypeName)
            .AppendLine(" : global::System.IDisposable");
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Reading.Logical.ParquetReader " + names.Helper("_reader") + ";");
        builder.AppendLine();
        builder.Append("        internal ").Append(readerTypeName)
            .AppendLine("(global::Plank.Reading.Logical.ParquetReader reader)");
        builder.AppendLine("            => " + names.Helper("_reader") + " = reader ?? throw new global::System.ArgumentNullException(nameof(reader));");
        builder.AppendLine();
        builder.AppendLine("        public global::Plank.Schema.ParquetSchema Schema => " + names.Helper("_reader") + ".Schema;");
        builder.AppendLine();
        builder.AppendLine("        public global::Plank.Reading.Logical.ParquetFileMetadata Metadata => " + names.Helper("_reader") + ".Metadata;");
        builder.AppendLine();
        builder.AppendLine("        public " + names.Root("ReadRowGroupCollection") + " RowGroups => new(" + names.Helper("_reader") + ".RowGroups);");
        builder.AppendLine();
        ...
        builder.AppendLine("            => " + names.Helper("_reader") + ".Reset(stream, pagePruner);");
        builder.AppendLine();
        ...
        builder.AppendLine("            => " + names.Helper("_reader") + ".Reset(source, pagePruner);");
        builder.AppendLine();
        builder.AppendLine("        public void Dispose()");
        builder.AppendLine("            => " + names.Helper("_reader") + ".Dispose();");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    public readonly struct " + names.Root("ReadRowGroupCollection"));
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Reading.Logical.RowGroupCollection " + names.Helper("_rowGroups") + ";");
        builder.AppendLine();
        builder.AppendLine("        internal " + names.Root("ReadRowGroupCollection") + "(global::Plank.Reading.Logical.RowGroupCollection rowGroups)");
        builder.AppendLine("            => " + names.Helper("_rowGroups") + " = rowGroups;");
        builder.AppendLine();
        builder.AppendLine("        public int Count => " + names.Helper("_rowGroups") + ".Count;");
        builder.AppendLine();
        builder.AppendLine("        public " + names.Root("ReadRowGroup") + " this[int index] => new(" + names.Helper("_rowGroups") + "[index]);");
        builder.AppendLine();
        builder.AppendLine("        public Enumerator GetEnumerator()");
        builder.AppendLine("            => new(" + names.Helper("_rowGroups") + ".GetEnumerator());");
        builder.AppendLine();
        ...
        builder.AppendLine();
        builder.AppendLine("            public " + names.Root("ReadRowGroup") + " Current => new(_inner.Current);");
        builder.AppendLine();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    public readonly struct " + names.Root("ReadRowGroup"));
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.Reading.Logical.RowGroup " + names.Helper("_rowGroup") + ";");
        builder.AppendLine();
        builder.AppendLine("        internal " + names.Root("ReadRowGroup") + "(global::Plank.Reading.Logical.RowGroup rowGroup)");
        builder.AppendLine("            => " + names.Helper("_rowGroup") + " = rowGroup;");
        builder.AppendLine();
        builder.AppendLine("        public int Index => " + names.Helper("_rowGroup") + ".Index;");
        builder.AppendLine();
        builder.AppendLine("        public ulong MetadataOffset => " + names.Helper("_rowGroup") + ".MetadataOffset;");
        builder.AppendLine();
        builder.AppendLine("        public ulong ColumnChunkOffset => " + names.Helper("_rowGroup") + ".ColumnChunkOffset;");
        builder.AppendLine();
        builder.AppendLine("        public ulong RowCount => " + names.Helper("_rowGroup") + ".RowCount;");
        for (var i = 0; i < columns.Length; i++)
        {
            builder.AppendLine();
                ...
                IsGuidClr(columns[i].ClrTypeName))
            {
                builder.Append("        public global::Plank.Reading.Logical.RowGroupColumn<byte> ")
                    ...
                    .Append("Column => " + names.Helper("_rowGroup") + ".Column<byte>(").Append(schemaMemberName).Append(".LeafColumns[")
                    .Append(i).AppendLine("]);");
            }
            else
            {
                builder.Append("        public global::Plank.Reading.Logical.RowGroupColumn<")
                    ...
                    .Append("Column => " + names.Helper("_rowGroup") + ".Column<").Append(columns[i].ClrTypeName)
                    .Append(">(").Append(schemaMemberName).Append(".LeafColumns[").Append(i).AppendLine("]);");
            }
        }
        builder.AppendLine("    }");
    ...
    static void AppendRowApiProjection(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("        " + names.Root("Projection") + "(global::Plank.RowApi.RowApiColumnDescriptor[] columns)");
        builder.AppendLine("            => " + names.Helper("_columns") + " = columns;");
        builder.AppendLine();
        ...
        {
            builder.AppendLine();
            builder.Append("        public static " + names.Root("Projection") + " ").Append(EscapeIdentifier(columns[i].PropertyName))
                .Append(" { get; } = new([")
                .Append(names.Helper(GetRowApiColumnFieldName(columns[i].PropertyName))).AppendLine("]);");
        }
        builder.AppendLine();
        builder.AppendLine("        public static " + names.Root("Projection") + " operator |(" + names.Root("Projection") + " left, " + names.Root("Projection") + " right)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (left." + names.Helper("_columns") + " is null || right." + names.Helper("_columns") + " is null)");
        builder.AppendLine("                return " + names.Get("projection:All", "All") + ";");
        builder.AppendLine("            if (left." + names.Helper("_columns") + ".Length == 0)");
        builder.AppendLine("                return right;");
        builder.AppendLine("            if (right." + names.Helper("_columns") + ".Length == 0)");
    ...
    static void AppendRowReader(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns, string schemaMemberName)
    {
        ...
        builder.AppendLine("        {");
        builder.Append("            " + names.Helper("_core") + " = new global::Plank.RowApi.RowReaderCore(stream, ").Append(schemaMemberName)
            .AppendLine(", " + names.Root("s_rowApiColumns") + ", projection." + names.Get("projection:Columns", "Columns") + ", options, schemaEvolution);");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine("        {");
        builder.Append("            " + names.Helper("_core") + " = new global::Plank.RowApi.RowReaderCore(source, ").Append(schemaMemberName)
            .AppendLine(", " + names.Root("s_rowApiColumns") + ", projection." + names.Get("projection:Columns", "Columns") + ", options, schemaEvolution);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public Enumerator GetEnumerator()");
        ...
        builder.AppendLine();
        builder.AppendLine("        public void Reset(global::System.IO.Stream stream, " + names.Root("Projection") + " projection = default, global::Plank.Reading.ParquetSchemaEvolutionOptions? schemaEvolution = null)");
        builder.AppendLine("            => " + names.Helper("_core") + ".Reset(stream, projection." + names.Get("projection:Columns", "Columns") + ", schemaEvolution);");
        builder.AppendLine();
        builder.AppendLine("        public void Reset(global::Plank.Reading.IParquetReadSource source, " + names.Root("Projection") + " projection = default, global::Plank.Reading.ParquetSchemaEvolutionOptions? schemaEvolution = null)");
        builder.AppendLine("            => " + names.Helper("_core") + ".Reset(source, projection." + names.Get("projection:Columns", "Columns") + ", schemaEvolution);");
        builder.AppendLine();
        ...
        builder.AppendLine();
        builder.AppendLine("            internal Enumerator(" + names.Root("RowReader") + " reader)");
        builder.AppendLine("                => " + names.Helper("_reader") + " = reader ?? throw new global::System.ArgumentNullException(nameof(reader));");
        builder.AppendLine();
        builder.AppendLine("            public " + names.Root("ReadRow") + " Current => " + names.Helper("_reader") + ".Current;");
        builder.AppendLine();
        builder.AppendLine("            public bool MoveNext()");
        builder.AppendLine("                => " + names.Helper("_reader") + ".MoveNext();");
        builder.AppendLine();
        builder.AppendLine("            public void Dispose()");
        builder.AppendLine("            {");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public " + names.Root("ReadRow") + " Current");
        builder.AppendLine("        {");
        builder.AppendLine("            get");
        builder.AppendLine("            {");
        builder.AppendLine("                " + names.Helper("_core") + ".ThrowIfNotPositioned();");
        builder.AppendLine("                return new " + names.Root("ReadRow") + "(" + names.Helper("_core") + ");");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public bool MoveNext()");
        builder.AppendLine("            => " + names.Helper("_core") + ".MoveNext();");
        builder.AppendLine();
        builder.AppendLine("        public void Dispose()");
        builder.AppendLine("            => " + names.Helper("_core") + ".Dispose();");
        builder.AppendLine("    }");
        builder.AppendLine();
        AppendReadRow(builder, names, columns);
    }

    static void AppendReadRow(StringBuilder builder, GeneratedMemberNames names, ImmutableArray<MappedColumn> columns)
    {
        builder.AppendLine("    public readonly ref struct " + names.Root("ReadRow"));
        builder.AppendLine("    {");
        builder.AppendLine("        readonly global::Plank.RowApi.RowReaderCore " + names.Helper("_core") + ";");
        builder.AppendLine();
        builder.AppendLine("        internal " + names.Root("ReadRow") + "(global::Plank.RowApi.RowReaderCore core)");
        builder.AppendLine("            => " + names.Helper("_core") + " = core;");
        builder.AppendLine();
        for (var i = 0; i < columns.Length; i++)
        {
            var propertyName = columns[i].PropertyName;
            var descriptorName = names.Helper(GetRowApiColumnFieldName(propertyName));
            if (IsStringClr(columns[i].ClrTypeName))
            {
                ...
                builder.AppendLine("            get");
                builder.AppendLine("            {");
                builder.Append("                var value = " + names.Helper("_core") + ".GetCurrentBinary(").Append(descriptorName)
                    .AppendLine(");");
            ...
            else if (IsUtf8ByteArrayClr(columns[i].ClrTypeName))
            {
                builder.Append("        public global::Plank.RowApi.RowReaderBinaryValue ")
                    .Append(EscapeIdentifier(propertyName)).AppendLine();
                builder.Append("            => " + names.Helper("_core") + ".GetCurrentBinary(").Append(descriptorName)
                    .AppendLine(");");
            }
            else
            {
                builder.Append("        public ref ").Append(columns[i].ClrTypeName).Append(' ')
                    .Append(EscapeIdentifier(propertyName)).AppendLine();
                builder.Append("            => ref " + names.Helper("_core") + ".GetCurrent<").Append(columns[i].ClrTypeName).Append(">(")
                    .Append(i).AppendLine(");");
            }
            if (i < columns.Length - 1)
                builder.AppendLine();
    ...
    static string EscapeIdentifier(string value)
        => Microsoft.CodeAnalysis.CSharp.SyntaxFacts.GetKeywordKind(value) ==
           ...
           Microsoft.CodeAnalysis.CSharp.SyntaxFacts.GetContextualKeywordKind(value) ==
               Microsoft.CodeAnalysis.CSharp.SyntaxKind.None
            ? value
            : $"@{value}";

    static string? GetNamespaceName(INamespaceSymbol namespaceSymbol)
    {
    ...
    static bool TryGetConverterSpec(IPropertySymbol property, out ConverterSpec? converter, out string error)
    {
        ...
        converter = new ConverterSpec(
            converterType.ToDisplayString(EscapedFullyQualifiedTypeNameFormat), physicalClrTypeName);
        return true;
    }
