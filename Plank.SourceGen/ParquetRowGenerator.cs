...
public sealed class ParquetRowGenerator : IIncrementalGenerator
{
    ...
    static string BuildSource(IPropertySymbol schemaProperty, ImmutableArray<MappedColumn> columns)
    {
        ...
        builder.AppendLine("        => new(writer, rowCount, writer.RowApiMaxParallelism);");
        builder.AppendLine();
        builder.AppendLine("    public static PipelineWriter CreatePipelineWriter(global::Plank.Writing.ParquetWriter writer, int rowCount, uint maxParallelism)");
        builder.AppendLine("        => new(writer, rowCount, maxParallelism);");
        builder.AppendLine();
        ...
        builder.AppendLine("            _slot = new BufferSlot(rowGroupWriter, rowCount);");
        ...
        builder.AppendLine("        readonly global::System.Collections.Generic.Queue<BufferSlot> _readySlots;");
        builder.AppendLine("        readonly global::System.Collections.Generic.Queue<BufferSlot> _freeSlots;");
        builder.AppendLine("        readonly global::System.Threading.Thread[] _workers;");
        builder.AppendLine("        readonly object _gate;");
        builder.AppendLine("        readonly object _writeGate;");
        builder.AppendLine("        BufferSlot _active;");
        builder.AppendLine("        ulong _nextQueuedSequence;");
        builder.AppendLine("        ulong _nextWriteSequence;");
        builder.AppendLine("        bool _addingCompleted;");
        ...
        builder.AppendLine();
        builder.AppendLine("        internal PipelineWriter(global::Plank.Writing.ParquetWriter writer, int rowCount, uint maxParallelism)");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("            if (maxParallelism > int.MaxValue)");
        builder.AppendLine("                throw new global::System.ArgumentOutOfRangeException(nameof(maxParallelism), maxParallelism, $\"Max parallelism must be <= {int.MaxValue}.\");");
        builder.AppendLine();
        builder.AppendLine("            var workerCount = checked((int)maxParallelism);");
        builder.AppendLine("            _readySlots = new global::System.Collections.Generic.Queue<BufferSlot>(workerCount);");
        builder.AppendLine("            _freeSlots = new global::System.Collections.Generic.Queue<BufferSlot>(workerCount);");
        builder.AppendLine("            _workers = new global::System.Threading.Thread[workerCount];");
        builder.AppendLine("            _gate = new object();");
        builder.AppendLine("            _writeGate = new object();");
        builder.AppendLine("            _nextQueuedSequence = 0;");
        builder.AppendLine("            _nextWriteSequence = 0;");
        builder.AppendLine("            _addingCompleted = false;");
        builder.AppendLine("            _completed = false;");
        builder.AppendLine("            _fault = null;");
        builder.AppendLine();
        builder.AppendLine("            for (var i = 0; i < workerCount; i++)");
        builder.AppendLine("                _freeSlots.Enqueue(new BufferSlot(writer, rowCount));");
        builder.AppendLine();
        builder.AppendLine("            _active = _freeSlots.Dequeue();");
        builder.AppendLine();
        builder.AppendLine("            for (var i = 0; i < _workers.Length; i++)");
        builder.AppendLine("            {");
        builder.AppendLine("                _workers[i] = new global::System.Threading.Thread(WorkerLoop)");
        builder.AppendLine("                {");
        builder.AppendLine("                    IsBackground = true,");
        builder.AppendLine("                    Name = $\"PlankRowApiWorker-{i}\"");
        builder.AppendLine("                };");
        builder.AppendLine("                _workers[i].Start();");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public Row GetRow()");
        builder.AppendLine("        {");
        builder.AppendLine("            lock (_gate)");
        builder.AppendLine("            {");
        ...
        builder.AppendLine();
        builder.AppendLine("                QueueActiveSlotNoLock();");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine("                {");
        builder.AppendLine("                    _active.Sequence = _nextQueuedSequence++;");
        builder.AppendLine("                    _readySlots.Enqueue(_active);");
        builder.AppendLine("                }");
        builder.AppendLine();
        builder.AppendLine("                _completed = true;");
        builder.AppendLine("                _addingCompleted = true;");
        builder.AppendLine("                global::System.Threading.Monitor.PulseAll(_gate);");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            for (var i = 0; i < _workers.Length; i++)");
        builder.AppendLine("                _workers[i].Join();");
        builder.AppendLine();
        builder.AppendLine("            ThrowIfFaulted();");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        void QueueActiveSlotNoLock()");
        builder.AppendLine("        {");
        builder.AppendLine("            _active.Sequence = _nextQueuedSequence++;");
        builder.AppendLine("            _readySlots.Enqueue(_active);");
        builder.AppendLine("            global::System.Threading.Monitor.PulseAll(_gate);");
        builder.AppendLine("            _active = TakeFreeSlotNoLock();");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        BufferSlot TakeFreeSlotNoLock()");
        builder.AppendLine("        {");
        builder.AppendLine("            while (_freeSlots.Count == 0)");
        builder.AppendLine("            {");
        builder.AppendLine("                ThrowIfFaulted();");
        builder.AppendLine("                global::System.Threading.Monitor.Wait(_gate);");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            return _freeSlots.Dequeue();");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        void WorkerLoop()");
        builder.AppendLine("        {");
        builder.AppendLine("            while (true)");
        builder.AppendLine("            {");
        builder.AppendLine("                BufferSlot slot;");
        builder.AppendLine("                lock (_gate)");
        builder.AppendLine("                {");
        builder.AppendLine("                    while (_readySlots.Count == 0 && !_addingCompleted && _fault is null)");
        builder.AppendLine("                        global::System.Threading.Monitor.Wait(_gate);");
        builder.AppendLine();
        builder.AppendLine("                    if (_readySlots.Count == 0)");
        builder.AppendLine("                        return;");
        builder.AppendLine();
        builder.AppendLine("                    slot = _readySlots.Dequeue();");
        builder.AppendLine("                }");
        builder.AppendLine();
        builder.AppendLine("                try");
        builder.AppendLine("                {");
        builder.AppendLine("                    slot.SerializeColumns();");
        builder.AppendLine("                    lock (_writeGate)");
        builder.AppendLine("                    {");
        builder.AppendLine("                        while (slot.Sequence != _nextWriteSequence && _fault is null)");
        builder.AppendLine("                            global::System.Threading.Monitor.Wait(_writeGate);");
        builder.AppendLine();
        builder.AppendLine("                        ThrowIfFaulted();");
        builder.AppendLine("                        var rowGroupWriter = _writer.StartRowGroup();");
        builder.AppendLine("                        slot.WriteSerialized(rowGroupWriter);");
        builder.AppendLine("                        _nextWriteSequence++;");
        builder.AppendLine("                        global::System.Threading.Monitor.PulseAll(_writeGate);");
        builder.AppendLine("                    }");
        builder.AppendLine("                }");
        builder.AppendLine("                catch (global::System.Exception ex)");
        builder.AppendLine("                {");
        builder.AppendLine("                    RecordFault(ex);");
        builder.AppendLine("                }");
        ...
        builder.AppendLine("                    lock (_gate)");
        builder.AppendLine("                    {");
        builder.AppendLine("                        _freeSlots.Enqueue(slot);");
        builder.AppendLine("                        global::System.Threading.Monitor.PulseAll(_gate);");
        builder.AppendLine("                    }");
        builder.AppendLine("                }");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        void RecordFault(global::System.Exception exception)");
        builder.AppendLine("        {");
        builder.AppendLine("            var captured = global::System.Runtime.ExceptionServices.ExceptionDispatchInfo.Capture(exception);");
        builder.AppendLine("            if (global::System.Threading.Interlocked.CompareExchange(ref _fault, captured, null) is not null)");
        builder.AppendLine("                return;");
        builder.AppendLine();
        builder.AppendLine("            lock (_gate)");
        builder.AppendLine("            {");
        ...
        builder.AppendLine("        {");
        builder.AppendLine("            var fault = _fault;");
        builder.AppendLine("            if (fault is not null)");
        builder.AppendLine("                fault.Throw();");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.AppendLine("        internal ulong Sequence;");
