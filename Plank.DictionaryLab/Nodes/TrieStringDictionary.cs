...
public sealed class TrieStringDictionary : IExperimentDictionary<string>
{
    public static string ExperimentName => "tree.trie.v1";

    public static Type? ParentExperimentType => null;

    public static string ExperimentDescription => "Character trie with per-node linked child edges.";
    ...
    public int GetOrAddIndex(string key)
    {
        ...
        for (var i = 0; i < key.Length; i++)
            nodeIndex = GetOrAddChild(nodeIndex, key[i]);
    ...
    int GetOrAddChild(int nodeIndex, char label)
    {
