...
public sealed class SortedArrayLengthFirstFirstUnitStringDictionary : IExperimentDictionary<string>
{
    public static readonly string Name = "tree.sorted-array.length-first.first-unit.v1";

    public static readonly Type? Parent = typeof(SortedArrayLengthFirstStringDictionary);

    public static readonly string Description = "Sorted array length-first branch with first-char metadata short-circuiting compare.";
    ...
    public int GetOrAddIndex(string key)
    {
        ...
        while (low <= high)
        {
            ...
            if (compare == 0)
                compare = keyFirst.CompareTo(_firstChars[mid]);
