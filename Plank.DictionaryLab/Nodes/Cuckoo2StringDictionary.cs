...
public sealed class Cuckoo2StringDictionary : IExperimentDictionary<string>
{
    public static string ExperimentName => "hash.cuckoo2.v1";

    public static Type? ParentExperimentType => null;

    public static string ExperimentDescription => "Two-choice cuckoo hashing with bounded eviction and resize fallback.";
    ...
    public int GetOrAddIndex(string key)
    {
        ...
        if (entryA != 0 && _keys[entryA - 1] == key)
            return entryA - 1;

        var entryB = _slotsB[slotB];
        if (entryB != 0 && _keys[entryB - 1] == key)
            return entryB - 1;
