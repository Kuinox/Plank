...
public sealed class RobinHoodTaggedSparseUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "hash.robinhood.tagged.sparse.v1";

    public static Type? ParentExperimentType => typeof(RobinHoodTaggedUtf8Dictionary);

    public static string ExperimentDescription => "UTF-8 Robin Hood tagged probing with 40% max load.";
    ...
    public sealed class MixedUniquenessThroughput10To70Metadata : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        public static string ExperimentName => "hash.robinhood.tagged.sparse.mixed-uniqueness-10-70.meta.v1";

        public static Type? ParentExperimentType => typeof(RobinHoodTaggedSparseUtf8Dictionary);

        public static string ExperimentDescription => "UTF-8 Robin Hood tagged sparse probing with epoch-stamped recent-hit metadata tuned for mixed uniqueness throughput (10-70).";
        ...
        bool TryGetRecentHit(ReadOnlyMemory<byte> key, int hash, out int index)
        {
            ...
            if (!_keys[cachedIndex].Span.SequenceEqual(key.Span))
            {
                index = -1;
                return false;
            }
        ...
        bool TryFindIndex(ReadOnlyMemory<byte> key, int hash, byte tag, out int index)
        {
            ...
            while (true)
            {
                ...
                var existingIndex = entry - 1;
                if (_tags[slot] == tag && _keys[existingIndex].Span.SequenceEqual(keySpan))
                {
                    index = existingIndex;
                    return true;
                }
...
public sealed class RobinHoodTaggedUltraSparseUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "hash.robinhood.tagged.ultra-sparse.v1";

    public static Type? ParentExperimentType => typeof(RobinHoodTaggedSparseUtf8Dictionary);

    public static string ExperimentDescription => "UTF-8 Robin Hood tagged probing with 25% max load.";
    ...
    public sealed class SwapReduced : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        public static string ExperimentName => "hash.robinhood.tagged.ultra-sparse.tuned.swap-reduced.v1";

        public static Type? ParentExperimentType => typeof(RobinHoodTaggedUltraSparseUtf8Dictionary);

        public static string ExperimentDescription => "UTF-8 Robin Hood tagged ultra-sparse probing tuned to reduce swap and probe-distance churn during inserts.";
    ...
    public sealed class SwapReducedProbeReuse : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        ...
        public static string ExperimentDescription => "UTF-8 Robin Hood tagged ultra-sparse probing that reuses miss probe state to avoid duplicate insert-path scans.";
        ...
        bool TryFindIndex(ReadOnlyMemory<byte> key, int hash, byte tag, out int index, out int insertSlot, out int insertDistance)
        {
            var slots = _slots;
            var distance = _distance;
            var tags = _tags;
            ...
            while (true)
            {
                ...
                {
                    var existingIndex = slots[slot] - 1;
                    if (keys[existingIndex].Span.SequenceEqual(keySpan))
                    {
                        index = existingIndex;
                        insertSlot = -1;
                        insertDistance = 0;
    ...
    public sealed class SwapReducedProbeReusePackedMeta : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        ...
        public static string ExperimentDescription => "UTF-8 Robin Hood tagged ultra-sparse probing that packs tag+distance metadata to reduce probe-path memory traffic.";
    ...
    public sealed class SwapReducedProbeReusePackedMetaResetTouched : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        ...
        public static string ExperimentDescription => "UTF-8 Robin Hood tagged ultra-sparse probing with packed metadata and touched-slot reset clearing to reduce reset-path memory work.";
    ...
    public sealed class Tuned : IExperimentDictionary<ReadOnlyMemory<byte>>
    {
        ...
        public static string ExperimentDescription => "UTF-8 Robin Hood tagged ultra-sparse probing tuned for tag-first early checks.";
