...
public sealed class SortedArrayUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "tree.sorted-array.v1";

    public static Type? ParentExperimentType => null;

    public static string ExperimentDescription => "UTF-8 sorted array reference baseline.";
    ...
    public int GetOrAddIndex(ReadOnlyMemory<byte> key)
    {
        ...
        while (low <= high)
        {
            ...
            var compare = _sorted[mid].Span.SequenceCompareTo(key.Span);
            if (compare == 0)
                return _indices[mid];
            if (compare < 0)
                low = mid + 1;
