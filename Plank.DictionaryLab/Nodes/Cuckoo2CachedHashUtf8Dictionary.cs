...
public sealed class Cuckoo2CachedHashUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "hash.cuckoo2.cached-hash.v1";

    public static Type? ParentExperimentType => typeof(Cuckoo2Utf8Dictionary);

    public static string ExperimentDescription => "UTF-8 two-choice cuckoo hashing with cached per-entry hash and hash-prefiltered equality.";
    ...
    public int GetOrAddIndex(ReadOnlyMemory<byte> key)
    {
        ...
        {
            var indexA = entryA - 1;
            if (_hashes[indexA] == hash1 && _keys[indexA].Span.SequenceEqual(key.Span))
                return indexA;
        }
        ...
        {
            var indexB = entryB - 1;
            if (_hashes[indexB] == hash1 && _keys[indexB].Span.SequenceEqual(key.Span))
                return indexB;
        }
