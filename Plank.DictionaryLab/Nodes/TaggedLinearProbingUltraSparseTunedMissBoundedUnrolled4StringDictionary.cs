...
public sealed class TaggedLinearProbingUltraSparseTunedMissBoundedUnrolled4StringDictionary : IExperimentDictionary<string>
{
    public static string ExperimentName => "hash.linear.tagged.ultra-sparse.tuned.miss-bounded.unrolled4.v1";

    public static Type? ParentExperimentType => typeof(TaggedLinearProbingUltraSparseStringDictionary.Tuned.MissBounded);

    public static string ExperimentDescription => "Tagged ultra-sparse tuned miss-bounded probing with 4-step unrolled probe loop.";
    ...
    int Insert(int home, int slot, int probeDistance, string key, byte tag, int[] slots, byte[] tags, string[] keys, ushort[] maxDistances)
    {
        var index = Count++;
        keys[index] = key;
        slots[slot] = index + 1;
