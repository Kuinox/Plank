...
public sealed class Cuckoo2CachedHashLengthPrefilterStringDictionary : IExperimentDictionary<string>
{
    public static string ExperimentName => "hash.cuckoo2.cached-hash.length-prefilter.v1";

    public static Type? ParentExperimentType => typeof(Cuckoo2CachedHashStringDictionary);

    public static string ExperimentDescription => "Two-choice cuckoo hashing with cached hash and length prefilter before string equality.";
    ...
    public int GetOrAddIndex(string key)
    {
        ...
        {
            var indexA = entryA - 1;
            if (_hashes[indexA] == hash1 && _lengths[indexA] == keyLength && _keys[indexA] == key)
                return indexA;
        }
        ...
        {
            var indexB = entryB - 1;
            if (_hashes[indexB] == hash1 && _lengths[indexB] == keyLength && _keys[indexB] == key)
                return indexB;
        }
