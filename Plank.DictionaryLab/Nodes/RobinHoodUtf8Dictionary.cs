...
public sealed class RobinHoodUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    ...
    public static string ExperimentDescription => "UTF-8 Robin Hood probing baseline.";
    ...
    bool TryFindIndex(ReadOnlyMemory<byte> key, int hash, out int index)
    {
    ...
    void ResizeTo(int slotLength)
    {
        ...
        for (var i = 0; i < oldCount; i++)
            InsertIndex(i, ByteHashing.Hash(oldKeys[i].Span) & int.MaxValue);
    }
