...
public sealed class Cuckoo2CachedHashLengthPrefilterUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "hash.cuckoo2.cached-hash.length-prefilter.v1";

    public static Type? ParentExperimentType => typeof(Cuckoo2CachedHashUtf8Dictionary);

    public static string ExperimentDescription => "UTF-8 two-choice cuckoo hashing with cached hash and length prefilter before sequence equality.";
    ...
    public int GetOrAddIndex(ReadOnlyMemory<byte> key)
    {
        ...
        {
            var indexA = entryA - 1;
            if (_hashes[indexA] == hash1 && _lengths[indexA] == keyLength && _keys[indexA].Span.SequenceEqual(key.Span))
                return indexA;
        }
        ...
        {
            var indexB = entryB - 1;
            if (_hashes[indexB] == hash1 && _lengths[indexB] == keyLength && _keys[indexB].Span.SequenceEqual(key.Span))
                return indexB;
        }
