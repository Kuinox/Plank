...
public sealed class ChainedHashUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static string ExperimentName => "hash.chained.v1";

    public static Type? ParentExperimentType => null;

    public static string ExperimentDescription => "UTF-8 separate chaining with index-linked entry lists.";
    ...
    public int GetOrAddIndex(ReadOnlyMemory<byte> key)
    {
        ...
        while (entry >= 0)
        {
            if (_keys[entry].Span.SequenceEqual(key.Span))
                return entry;
            entry = _next[entry];
        }
    ...
    void ResizeBuckets(int bucketLength)
    {
        ...
        {
            var bucket = (ByteHashing.Hash(_keys[i].Span) & int.MaxValue) & mask;
