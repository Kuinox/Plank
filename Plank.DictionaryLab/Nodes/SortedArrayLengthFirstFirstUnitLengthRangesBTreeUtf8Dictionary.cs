...
public sealed class SortedArrayLengthFirstFirstUnitLengthRangesBTreeUtf8Dictionary : IExperimentDictionary<ReadOnlyMemory<byte>>
{
    public static readonly string Name = "tree.sorted-array.length-first.first-unit.length-ranges.btree.v1";

    public static readonly Type? Parent = typeof(SortedArrayLengthFirstFirstUnitLengthRangesUtf8Dictionary);

    public static readonly string Description = "UTF-8 sorted array length-first+first-byte ranges with active-length indexing to reduce insert-path range maintenance.";
    ...
    public int GetOrAddIndex(ReadOnlyMemory<byte> key)
    {
        ...
        {
            var firstLow = LowerBoundFirstByte(rangeLow, rangeHigh + 1, keyFirst);
            var firstHighExclusive = UpperBoundFirstByte(firstLow, rangeHigh + 1, keyFirst);
            if (firstLow < firstHighExclusive)
            {
                var keyIndex = BinarySearchKey(firstLow, firstHighExclusive - 1, keySpan);
                if (keyIndex >= 0)
    ...
    int LowerBoundFirstByte(int low, int highExclusive, byte keyFirst)
    {
        while (low < highExclusive)
        {
            var mid = low + ((highExclusive - low) >> 1);
            if (_firstBytes[mid] < keyFirst)
                low = mid + 1;
            else
                highExclusive = mid;
        }
    ...
    int UpperBoundFirstByte(int low, int highExclusive, byte keyFirst)
    {
        while (low < highExclusive)
        {
            var mid = low + ((highExclusive - low) >> 1);
    ...
    int BinarySearchKey(int low, int high, ReadOnlySpan<byte> key)
    {
        while (low <= high)
        {
            var mid = low + ((high - low) >> 1);
            var compare = key.SequenceCompareTo(_sorted[mid].Span);
            if (compare == 0)
                return mid;
            if (compare < 0)
                high = mid - 1;
