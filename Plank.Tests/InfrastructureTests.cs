...
internal sealed class InfrastructureTests
{
    ...
    public async Task ParquetLogNoneNoOpsAreCallable()
    {
        ...
        ParquetLog.None.StreamWriteObserved(64, 100, 250);
        ParquetLog.None.ColumnWriteMetricsObserved("A", 10, 10, 64, 11, 12, 13, 14);
    ...
    public async Task LoggerParquetLogEmitsMessagesForBothMetadataModes()
    {
        ...
        log.StreamWriteObserved(1024, 55, 21);
        log.ColumnWriteMetricsObserved("A", 10, 10, 1024, 20, 30, 40, 50);

        await Assert.That(logger.Entries.Count).IsEqualTo(5);
        ...
        await Assert.That(logger.Entries[3].Message.Contains("Stream write observed", StringComparison.Ordinal)).IsTrue();
        await Assert.That(logger.Entries[4].Message.Contains("Column metrics", StringComparison.Ordinal)).IsTrue();
    }
    ...
    [Test]
    public async Task WriterOptionsInitPropertiesAreSettable()
    {
        var pool = new NamedMemoryPool();
        IParquetLog log = new LoggerParquetLog(new CapturingLogger());
        var options = new ParquetWriterOptions
        {
            Log = log,
            BufferPool = pool,
            FooterBufferBytes = 4096,
            Compression = CompressionKind.Snappy
        };

        await Assert.That(ReferenceEquals(options.Log, log)).IsTrue();
        await Assert.That(ReferenceEquals(options.BufferPool, pool)).IsTrue();
        await Assert.That(options.FooterBufferBytes).IsEqualTo(4096);
        await Assert.That(options.Compression).IsEqualTo(CompressionKind.Snappy);
    }
