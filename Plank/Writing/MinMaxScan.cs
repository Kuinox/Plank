...
static class MinMaxScan
{
    ...
    internal static void ComputeVector128Bulk<T>(ReadOnlySpan<T> values, out T min, out T max)
        where T : struct, INumber<T>
    {
        ref var source = ref MemoryMarshal.GetReference(values);
        var length = (nuint)values.Length;
        var count = (nuint)Vector128<T>.Count;
        var begin = Vector128.LoadUnsafe(ref source);
        var end = Vector128.LoadUnsafe(ref source, length - count);
        Vector128<T> min0 = begin, min1 = end, min2 = begin, min3 = end;
        Vector128<T> max0 = begin, max1 = end, max2 = begin, max3 = end;

        nuint index = 0;
        var lastBlock = length - (count * Unroll);
        for (; index <= lastBlock; index += count * Unroll)
        {
            var value0 = Vector128.LoadUnsafe(ref source, index);
            var value1 = Vector128.LoadUnsafe(ref source, index + count);
            var value2 = Vector128.LoadUnsafe(ref source, index + (count * 2));
            var value3 = Vector128.LoadUnsafe(ref source, index + (count * 3));
            min0 = Vector128.Min(min0, value0);
            max0 = Vector128.Max(max0, value0);
            min1 = Vector128.Min(min1, value1);
            max1 = Vector128.Max(max1, value1);
            min2 = Vector128.Min(min2, value2);
            max2 = Vector128.Max(max2, value2);
            min3 = Vector128.Min(min3, value3);
            max3 = Vector128.Max(max3, value3);
        }

        for (; index + count <= length; index += count)
        {
            var value = Vector128.LoadUnsafe(ref source, index);
            min0 = Vector128.Min(min0, value);
            max0 = Vector128.Max(max0, value);
        }

        var minVector = Vector128.Min(Vector128.Min(min0, min1), Vector128.Min(min2, min3));
        var maxVector = Vector128.Max(Vector128.Max(max0, max1), Vector128.Max(max2, max3));
        min = minVector[0];
        max = maxVector[0];
        for (var lane = 1; lane < Vector128<T>.Count; lane++)
        {
            if (minVector[lane] < min)
                min = minVector[lane];
            if (maxVector[lane] > max)
    ...
    [MethodImpl(MethodImplOptions.NoInlining | MethodImplOptions.AggressiveOptimization)]
    internal static void CopyAndComputeVector128Bulk<T>(ReadOnlySpan<T> values, Span<T> destination, out T min,
        out T max)
        where T : struct, INumber<T>
    {
        ref var source = ref MemoryMarshal.GetReference(values);
        ref var target = ref MemoryMarshal.GetReference(destination);
        var length = (nuint)values.Length;
        var width = (nuint)Vector128<T>.Count;
        var min0 = Vector128.LoadUnsafe(ref source);
        var min1 = min0;
        var min2 = min0;
        var min3 = min0;
        var max0 = min0;
        ...
        {
            var value0 = Vector128.LoadUnsafe(ref source, index);
            var value1 = Vector128.LoadUnsafe(ref source, index + width);
            var value2 = Vector128.LoadUnsafe(ref source, index + width * 2);
            var value3 = Vector128.LoadUnsafe(ref source, index + width * 3);
            value0.StoreUnsafe(ref target, index);
            value1.StoreUnsafe(ref target, index + width);
            value2.StoreUnsafe(ref target, index + width * 2);
            value3.StoreUnsafe(ref target, index + width * 3);
