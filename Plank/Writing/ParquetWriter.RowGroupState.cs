...
public sealed partial class ParquetWriter
{
    internal sealed partial class RowGroupState
    {
        ...
        internal RowGroupState(ImmutableArray<Column> columns, ParquetWriterOptions options, IBufferPool bufferPool)
        {
            ...
            _options = options;
            _buffers = new RowGroupBufferCatalog(_columnStore.Columns, options.RowGroupOptions, options.RowGroupRowCountHint, options.Compression, bufferPool);
            _zstdCompressor = options.Compression == CompressionKind.Zstd ? new Compressor(1) : null;
            _pageHeaderBuffer = new byte[MaxPageHeaderBytes];
            _columnStore.RowCount = -1;
            _columnStore.NextOrdinal = 0;
            _drain.InProgress = 0;
        ...
        internal void Reset()
        {
            ...
            _drain.InProgress = 0;
            _drain.CompletionSignaled = 0;
            for (var i = 0; i < _columnStore.ColumnStates.Length; i++)
            {
                ref var state = ref _columnStore.ColumnStates[i];
                ...
                _columnStore.ColumnMetadata[i] = default;
                _columnStore.WriteSignals[i] = null;
            }
        }
        ...
        internal int ConfigureAndGetColumnCount(long startId)
        {
            ...
            return _columnStore.Columns.Length;
        }

        internal int GetRowCount()
            => _columnStore.RowCount;

        internal void CopyColumnMetadataTo(ColumnChunkMetadata[] destination)
            => Array.Copy(_columnStore.ColumnMetadata, destination, destination.Length);
        ...
        {
            if (!_columnStore.ColumnOrdinals.TryGetValue(column, out var ordinal))
        ...
        internal bool TryGetOrdinal(Column column, out int ordinal)
            => _columnStore.ColumnOrdinals.TryGetValue(column, out ordinal);
        ...
        {
            if (ordinal < 0 || ordinal >= _columnStore.ColumnStates.Length)
                throw new ArgumentOutOfRangeException(nameof(ordinal));
            if (Volatile.Read(ref _columnStore.ColumnStates[ordinal].WriteState) == WriteStateWritten)
                return ValueTask.CompletedTask;
            if (cancellationToken.IsCancellationRequested)
                return ValueTask.FromCanceled(cancellationToken);

            var signal = _columnStore.WriteSignals[ordinal];
            if (signal is null)
            {
                signal = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
                _columnStore.WriteSignals[ordinal] = signal;
            }
        ...
        internal void TryDrain(ParquetWriter writer)
        {
            while (true)
            {
                ...
                try
                {
                    while (_columnStore.NextOrdinal < _columnStore.ColumnStates.Length && Volatile.Read(ref _columnStore.ColumnStates[_columnStore.NextOrdinal].WriteState) == WriteStateEncoded)
                    {
                        WriteColumn(writer, _columnStore.NextOrdinal);
                        var signal = _columnStore.WriteSignals[_columnStore.NextOrdinal];
                        if (signal is not null)
                            signal.TrySetResult(true);
                        _columnStore.NextOrdinal++;
                    }

                    if (_columnStore.NextOrdinal == _columnStore.ColumnStates.Length && Interlocked.Exchange(ref _drain.CompletionSignaled, 1) == 0)
                ...
                if (Volatile.Read(ref _drain.CompletionSignaled) != 0)
                    return;
                if ((uint)_columnStore.NextOrdinal >= (uint)_columnStore.ColumnStates.Length)
                    return;
                if (Volatile.Read(ref _columnStore.ColumnStates[_columnStore.NextOrdinal].WriteState) != WriteStateEncoded)
                    return;
            }
        }
