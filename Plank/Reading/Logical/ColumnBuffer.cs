...
public readonly struct ColumnBuffer<T>
{
    ...
    internal ColumnBuffer(ParquetBuffer values, int valueCount, bool isVariableLength,
        ...
        IParquetBufferPool? borrowedBufferPool = null)
    {
    ...
    internal ColumnBuffer(ParquetBuffer values, int valueCount,
        ReadOnlyMemory<byte> variableLengthBorrowedPayload, IParquetBufferPool borrowedBufferPool)
    {
    ...
    public ParquetBuffer RetainValue(int index)
    {
        ...
        if (_isVariableLength)
        {
            ...
            {
                using var rented = _borrowedBufferPool!.Rent(checked((uint)descriptor.Length));
                descriptor.GetSpan(_variableLengthBorrowedArray, _variableLengthBorrowedOffset)
                    .CopyTo(rented.Span);
                return rented.RetainSlice(0, descriptor.Length);
            }
    ...
    internal IParquetBufferPool? BorrowedBufferPool
        => _borrowedBufferPool;
